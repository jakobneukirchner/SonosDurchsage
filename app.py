...
"""
SonosDurchsage v5
- Port wird sofort beim Import korrekt gesetzt
- MIME-Typen und Range-Requests für SONOS
- Robustes Gong+Durchsage-Playback
"""
...
# Port: aus Umgebungsvariable oder CLI-Argument – wird in __main__ gesetzt
...
def serve_media(filename):
    ...
    log.info(f"SONOS holt: {filename}  [{mimetype}]")
    ...
    try:
        ...
        for d in devices:
            try:
                ...
                log.warning(f"  {d.ip_address} Info-Fehler: {e}")
        return _discovered
    except Exception as e:
...
def _wait_until_done(dev, timeout: int = 20) -> None:
...
def play_on_sonos(speaker_ips: list, audio_path: str,
                  gong_path=None, volume=None) -> dict:
    if not SOCO_AVAILABLE:
        ...
        try:
            ...
            if gong_path:
                ...
                try:
                    ...
                    _wait_until_done(dev, timeout=20)
...
def api_status():
    return jsonify({"soco": SOCO_AVAILABLE, "audio": AUDIO_AVAILABLE,
                    "ip": local_ip(), "port": _flask_port, "version": "5.0"})
...
def api_play():
    ...
    if not audio_path:
        return jsonify({"success": False, "error": "Kein audio_path"})
    if not speakers:
        return jsonify({"success": False, "error": "Keine Lautsprecher ausgewählt"})
...
if __name__ == "__main__":
    ...
    # Port global setzen BEVOR Flask startet
    import builtins
    builtins.__dict__  # no-op; wir setzen direkt auf Modulebene:
    import app as _self
    _self._flask_port = port

    ip = local_ip()
    log.info("=" * 60)
    log.info(f"  SonosDurchsage v5")
